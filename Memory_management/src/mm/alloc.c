...
PRIVATE int allocationAlgorithm = FIRST_FIT_ALG;
...
/*===========================================================================*
 *				do_hole_map, do_worst_fit			     *
 *===========================================================================*/

PUBLIC int do_worst_fit(void) 
{
  if (mm_in.m1_i1 == FIRST_FIT_ALG || mm_in.m1_i1 == WORST_FIT_ALG)
    allocationAlgorithm = mm_in.m1_i1;
    return 0;
  return 1;
}
...
PUBLIC int do_hole_map(void)
{
  ...
  phys_clicks buffer[BUFFER_SIZE];
  int pairs = 0;
  int index = 0;
  hp = hole_head;
  ...
  while (hp != NIL_HOLE && hp->h_base < swap_base && index < BUFFER_SIZE - 1)
  {
    buffer[index++] = hp->h_len;
    buffer[index++] = hp->h_base;
    ++pairs;
    hp = hp->h_next;
  }
  buffer[index] = 0;

  /* Copy from kernel space to user program */
 ...
 */
  ...
  if (allocationAlgorithm == FIRST_FIT_ALG)
  {
    do {
      hp = hole_head;
  ...
  if (allocationAlgorithm == WORST_FIT_ALG)
  {
    do {
      hp = hole_head;
      max_ptr = hole_head;
      ...
      if(hp == NIL_HOLE && max_ptr->h_len >= clicks) {
