...
/*=====================================================================*
 *do_max_offspring, do_get_max_offspring_pid, do_debug_count_offspring *
 *=====================================================================*/
int count_offspring( int process_index ) 
{
	int children = 0;
	int i = 0;
	int counted = 0;
...
void max_offspring( int * offspring, pid_t * who, pid_t ignore )
{
	int max_offspring = -1;
	pid_t max_offspring_pid = -1;
	int counted_offspring = 0;
	int proc_nr = 0;
	for (proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr) {
		if( (mproc[proc_nr].mp_flags & IN_USE) && mproc[proc_nr].mp_pid != ignore) {
			/* 
			Process with pid 0 was exluded since it it connected to Memory Manager and 
			in majority of cases it had the most numerous offspring
			 */
			if( mproc[proc_nr].mp_pid != (pid_t)0) {
				counted_offspring = count_offspring( proc_nr );
				if( counted_offspring > max_offspring) {
					max_offspring = counted_offspring;
					max_offspring_pid = mproc[proc_nr].mp_pid;
				}
			}
		}
	}
	*offspring = max_offspring;
	*who = max_offspring_pid;
}

PUBLIC int do_max_offspring()
{
	int num_offspring = -1;
	pid_t main_parent_process_id = -1;
	max_offspring( & num_offspring, & main_parent_process_id, (pid_t)mm_in.m1_i1 );
	return(num_offspring);
}

PUBLIC int do_get_max_offspring_pid()
{
	int num_offspring = -1;
	pid_t main_parent_process_id = -1;
	max_offspring( & num_offspring, & main_parent_process_id, (pid_t)mm_in.m1_i1 );
	return((int)main_parent_process_id);
}

PUBLIC int do_debug_count_offspring()
{
	int i;

	for (i = 0; i < NR_PROCS; ++i) {
		if(mproc[i].mp_pid == (pid_t)mm_in.m1_i1) {
			return count_offspring( i );
		}
	}
	return(-1);
}
