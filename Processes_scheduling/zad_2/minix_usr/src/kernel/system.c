...
message *m_ptr;			/* pointer to request message */
{
  int process_id = m_ptr->m1_i1;
	int group_id = m_ptr->m1_i2;
	struct proc* rp;
  struct proc* this;
  struct proc* next;
	int exit_code = 1;

	if(group_id > MAXUSERGROUP || group_id < 0) {
		return 1;
	}

	for(rp = BEG_USER_ADDR; rp < END_PROC_ADDR; rp++) {
		if(!isemptyp(rp) && rp->p_pid == process_id) {
			rp->p_group_id = group_id;

      if(rp == rdy_head[USER_Q] && rp->p_nextready == NIL_PROC) {
        return 0;
      }
      ...
      while(1) {
        if(this->p_nextready == rp) {
          this->p_nextready = rp->p_nextready;
          if(rdy_tail[USER_Q] == rp) {
