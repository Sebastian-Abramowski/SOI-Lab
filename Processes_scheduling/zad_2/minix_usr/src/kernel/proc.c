 ...
 */
   ...
   * bound processes.)
   */
  if(rp->p_group_id == COMMON_GROUP || (rp->p_group_id == BATCH_GROUP && rdy_head[USER_Q]->p_group_id == BATCH_GROUP) || rdy_head[USER_Q] == NIL_PROC) {
    ...
    this = rdy_head[USER_Q];
    while(1) {
      if(this->p_nextready == NIL_PROC) {
        this->p_nextready = rp;
        rdy_tail[USER_Q] = rp;
        break;
      }
      if(this->p_nextready->p_group_id == BATCH_GROUP) {
        next = this->p_nextready;
        this->p_nextready = rp;
        rp->p_nextready = next;
        break;
      }
      this = this->p_nextready;
    }
  }
}
 ...
 */
  ...
  this = rdy_head[USER_Q];
  while(1) {
    if(this->p_group_id == COMMON_GROUP) {
      all_batch = 0;
      break;
    }
    if(this->p_nextready == NIL_PROC) {
  ...
  this = rdy_head[USER_Q];
  while(1) {
    if(this->p_group_id == BATCH_GROUP) {
      all_common = 0;
      break;
    }
    if(this->p_nextready == NIL_PROC) {
  ...
  this = rdy_head[USER_Q];
  if(this->p_group_id == COMMON_GROUP && this->p_nextready->p_group_id == BATCH_GROUP) {
    goto goto_out;
  }
  ...
  while(1) {
    ...
    if(this->p_nextready->p_nextready == NIL_PROC) {
      break;
    }

    if(this->p_nextready->p_nextready->p_group_id == BATCH_GROUP) {
      batch = this->p_nextready->p_nextready;
      first = rdy_head[USER_Q];
      this->p_nextready->p_nextready = first;
      rdy_head[USER_Q] = this->p_nextready;
      this->p_nextready = batch;
      goto goto_out;
    }
    this = this->p_nextready;
  }
